...
# 3. 打开教程，跟着做
#    tutorial.md 是从头到尾的线性教程，大约 3-7 天走完
```
...
| 做深度学习，想自定义 CUDA 算子 | [`tutorial.md` Part 5.3](./tutorial.md#53-动手接入-pytorch) → [`04_pytorch_extension/`](./04_pytorch_extension/) |
...
**核心文件**:
- [`tutorial.md`](./tutorial.md) — 主线教程（从零到写出 Softmax 算子）
- [`LEARNING_PATH.md`](./LEARNING_PATH.md) — 完整学习路线 + 文件结构索引
- [`DEBUG_AND_OPTIMIZE.md`](./DEBUG_AND_OPTIMIZE.md) — 调试与优化手册（代码出错或变慢时翻）
- [`EXERCISE_ANSWERS.md`](./EXERCISE_ANSWERS.md) — 练习题参考答案（先自己做！）
...
- NVIDIA GPU (本仓库示例默认面向 Compute Capability ≥ 7.0，即 Volta 及以上)
- CUDA Toolkit ≥ 11.0
- `nvcc` 在 PATH 中
...
Makefile 会自动检测你的 GPU 架构。如果检测失败，手动指定：
```bash
make all ARCH=sm_80   # A100
make all ARCH=sm_86   # RTX 3090
make all ARCH=sm_90   # H100
```
