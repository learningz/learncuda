...
//
// 编译: nvcc -O2 -o ex2_relu_level2 ex2_relu_level2.cu
// 运行: ./ex2_relu_level2
// 预期输出: "结果: ✓ 全部正确"
//
// 要求:
...
//
// 注意:
//   - x 是只读的输入, y 是输出 → 需要 2 块 GPU 内存
//   - x 的数据有正有负 (初始化代码已经写好了), y 应该把负数变成 0
//   - 想想: x 只需要传到 GPU, y 只需要传回 CPU — 和 vector_add 有什么不同?
// ============================================================
...
int main() {
    ...
    //   2. cudaMemcpy 把 h_x 传到 GPU (h_y 需要传吗? 为什么?)
